classdef PhotoEditorApp < handle
    %
    % Usage:
    %     app.PhotoEditorApp
    ...
    properties (Constant)
        AppPathBase = {'/PhotoEditor'};
        AppClass = 'PhotoEditorApp';
        Increment = 1;
        Decrement = 0;
        Version = '13a';
    end
    ...
    methods
        ...
        function startApp(obj)
            ...
            mlock;
            PhotoEditorApp.refcount(obj.Increment);
            ...
            existVal = exist(fullfile(pwd,'project')); %#ok<EXIST>
            ...
            if (doesShadowExist && ~pathOne.equals(pathTwo))
                % We are trying to execute the wrong project
                errordlg(message('MATLAB:apps:runapp:WrongEntryPoint', 'project').getString, ...
            ...
            if nargout(@project) == 0
                eval('project');
            else
                obj.AppHandle = eval('project');
            end
